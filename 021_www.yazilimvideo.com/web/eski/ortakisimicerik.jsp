<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>Yazılım Video</title>
<link rel="stylesheet" href="style.css" type="text/css" media="screen" />
<link rel="stylesheet" type="text/css" href="print.css" media="print" />
</head>

<body>


			<!--  ORTA KISIM ICERIK BASI  -->
		

				<div id="container">
					<div id="content">


						<article>
							<h1 id="">
								<a href="#"><img
									src="images/adsense_185666_adformat-display_468x60_en.jpg"
									alt="" class="aligncenter" /></a>
							</h1>
						</article>




						<article>
							<h2>
								<a href="index.jsp">&Uuml;cretsiz video eğitimlerine hoş geldiniz</a>
							</h2>
							<div class="entry">
							
								<a href="index.jsp"><img src="images/red1.jpg" alt=""
									class="alignleft" /></a>
									


							
								<p> <h3>SİTEMİZİN YAPIMINA ŞU ANDA DEVAM EDİYORUZ. Çok yakında da tamamlayacağız.İnşaAllah.</h3>  </p> 
								Amacımız bilişim alanında &uuml;lkemizin gelişimine katkı
									sağlamaktır. Bu sayfada yayınlanan ve yayınlancak olan bütün videolar daima ücretsiz olacaktır.
								 
									Sizler de videolarınızın yayınlanmasızını isterseniz. Bu sayfada yayınlayabilirsiniz.</p>
                                                            <h3>Örnek bir video</h3> 
								<div class="entry">
									<p>

										<iframe width="515" height="315"
											src="http://www.youtube.com/embed/DPS80jmLXB0"
											frameborder="0" allowfullscreen></iframe>


									</p>
								</div>


							</div>

						</article>





						<hr color="#990000"></hr>
						<br></br>

<!--
						<article>
							<h2>
								<a href="#">Öneri ve tavisyeler</a>
							</h2>
							<div class="entry">
								<a href="#"><img src="images/red2.jpg" alt=""
									class="alignleft" /></a>
								<p>Amacımız bilişim alanında &uuml;lkemizin gelişimine katkı
									sağlamaktır. Amacımız bilişim alanında &uuml;lkemizin
									gelişimine katkı sağlamaktır. Amacımız bilişim alanında
									&uuml;lkemizin gelişimine katkı sağlamaktır.</p>

								<p>


									<iframe width="515" height="315"
										src="http://www.youtube.com/embed/pjOBql72OMs?list=PL9A2D6C9F58E88E14&amp;hl=en_US"
										frameborder="0" allowfullscreen></iframe>
								</p>
							</div>

						</article>
-->
					</div>
					<!-- #content-->
				</div>
				<!-- #container-->
		<!--  ORTA KISIM ICERIK SONU  -->



</body>
</html>